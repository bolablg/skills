...
jobhunter-actions.csv        Append-only root action log and duplicate guard
...
> Use $job-hunter to onboard me progressively. Ask no more than three related questions at once. Help me select my résumé and optional professional resources, define ranked target markets, and confirm country-specific eligibility. Do not search or act externally yet.

Job Hunter requires one candidate-selected résumé. It can optionally use local evidence such as portfolio notes, certifications, publications, and writing samples, plus approved online resources such as LinkedIn, GitHub, a portfolio, personal site, research profile, or project demo. For each resource, onboarding records whether it may be read for evidence, used for tailoring/outreach, or shared with employers.

The starter intentionally leaves identity, roles, markets, and eligibility unanswered. Job Hunter asks progressively, shows a plain-language summary, and writes the YAML only after confirmation.
...
Current residence, desired markets, and legal eligibility are separate. Every broad region must list actual countries, remote roles must accept your residence country, and authorization/sponsorship is confirmed separately for each target country. Job Hunter never infers these facts from your nationality, résumé, language, or LinkedIn profile.

Before an application, account creation, recruiter message, or follow-up, Job Hunter checks `jobhunter-actions.csv`. A prior attempt blocks another attempt unless you explicitly approve the exact retry. The log also records research, preparation, approvals, handoffs, outcomes, and confirmations without storing secrets.
