...
2. If none exists, run `python3 scripts/init_project.py /path/to/project`.
3. Read [references/onboarding.md](references/onboarding.md). Conduct progressive onboarding when the profile is new, migrated, incomplete, or changing direction. Require one candidate-selected résumé; offer optional local and online evidence without scanning beyond the declared scope.
4. Summarize current residence, ranked target markets, remote/relocation rules, country-specific eligibility, resources, allowed uses, and unresolved facts. Ask the user to confirm before writing or changing the profile.
5. Read [references/project-profile.md](references/project-profile.md). Inventory only candidate-approved files and URLs in the declared project/resource scope.
6. Validate the profile for the requested mode with `scripts/validate_profile.py`.
7. Build or refresh `jobhunter-ledger/candidate-evidence.md` from `assets/candidate-evidence-template.md`. Cite the source and locator for each claim. Resolve resource conflicts with the candidate; never silently prefer résumé, LinkedIn, or portfolio wording.
8. Read [references/action-log.md](references/action-log.md), then read root `jobhunter-actions.csv` and the detailed ledgers before searching so work is not duplicated.
...
Search markets in configured priority order. Treat `candidate.current_location` as residence, not preference. Require explicit country codes for broad regions, and verify that every remote role accepts candidates residing in a configured residence country. Never infer authorization or target geography from a résumé, nationality, language, or profile URL.
...
Append each meaningful discovery, verification, scoring, preparation, approval, attempt, handoff, and outcome to root `jobhunter-actions.csv` as described in the action-log reference.
...
Immediately before submission, check the root action log for the canonical job key. Record `application-submit-attempt` with status `started` before the external action. Refuse an existing guarded attempt unless the user explicitly approves that exact retry.
...
6. Check the root action log using a privacy-minimized recruiter target key. Refuse a duplicate initial contact or follow-up unless the user explicitly approves that exact retry.
7. Record `recruiter-contact-attempt` or the numbered `recruiter-follow-up-attempt` before sending.
8. Send only approved messages through permitted channels. LinkedIn messages and connection requests are manual handoffs because LinkedIn prohibits third-party automation.
9. Respect the daily and follow-up limits. Stop immediately for `do-not-contact` or a decline.
