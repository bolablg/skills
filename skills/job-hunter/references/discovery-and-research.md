...
1. Read ranked `career_preferences.target_markets`; never derive target geography from current residence or candidate resources.
2. Convert target roles into title variants, skill combinations, industries, explicit countries/locations, work modes, and negative terms.
3. Search compliant public sources in market priority order: company career sites, public ATS feeds, search engines, professional associations, government job portals, and specialist recruiter sites.
4. For remote roles, verify the employer accepts candidates residing in at least one configured `candidate_residence_countries`; remote is not synonymous with worldwide.
5. Use employer-owned pages as the authority for whether a role is open and for its current description.
6. Canonicalize URLs, remove tracking parameters, and deduplicate by job ID or company + title + location.
7. Re-open every shortlisted role immediately before preparing or submitting an application.
8. Record market ID, source, discovery time, evidence-check time, and first decisive reason in the candidate ledger.
