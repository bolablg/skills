...
Read `references/action-log.md`. Check `jobhunter-actions.csv` using the canonical job key before touching the final submission flow. If the same guarded attempt already exists, stop for reconciliation or exact retry approval. Record the `application-submit-attempt` immediately before the external attempt, not afterward.
...
After submission, require a visible confirmation page/status or receipt. Append the outcome to the root action log and update the application ledger with timestamp, exact URL, confirmation text, résumé ID, and final status. Do not infer success from a button click. Never put passwords, recovery data, full government identifiers, or unnecessary demographics in the log, ledger, or screenshots.
