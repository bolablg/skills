...
Local visible tasks share the saved checkout. Before launching writers, Sol records the current status and gives each task exact, pairwise non-overlapping file or directory ownership. Each prompt forbids edits outside that ownership and forbids commits, pushes, pull requests, worktree operations, and destructive Git actions while tasks run concurrently. Workers preserve pre-existing changes, report every changed path, and stop if an unowned change is required. Run overlapping or dependent work serially. Track the returned `threadId` and `hostId`; use `wait_threads` for progress and `read_thread` only when additional detail is needed. The original Sol task remains the coordinator and owns integration, repository-wide verification, and publishing. Do not use `spawn_agent` for this visible-task route.
