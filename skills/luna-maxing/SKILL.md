---
name: luna-maxing
description: Coordinate a complex task with GPT-5.6 Sol as the aggregator and bounded GPT-5.6 Luna workers at max reasoning, using native subagents when routing is enforceable or isolated Codex CLI sessions with auditable requested settings. Use when the user asks for Luna Maxing, Sol/Luna orchestration, parallel deep reasoning, several independent research or implementation threads, or lower-cost high-volume delegation with a senior coordinator. Also use to create portable work packets for hosts that cannot route these exact models.
license: MIT
---
...
4. Create two or three independent work packets by default. Read [references/task-decomposition.md](references/task-decomposition.md) when boundaries, dependencies, or write ownership are non-trivial.
5. Select and verify a route using the routing rules below.
6. Launch bounded workers. Never let a worker coordinate the project or spawn more agents.
7. Wait for all useful packets, then validate their claims and artifacts. Read [references/verification-and-cost-controls.md](references/verification-and-cost-controls.md) for consequential or expensive work.
...
Use the host's native thread or subagent tools only if their current schema explicitly permits both `gpt-5.6-luna` and reasoning effort `max`. Set those values on every worker. Capture the returned task/thread identifier and reported usage when available.
...
- Do not exceed six concurrent workers or twelve total packets with the supplied runner; prefer fewer.
